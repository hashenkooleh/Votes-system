const e = (e, p, c) => {
  ...
  for (let prop in p) if (p.hasOwnProperty(prop)) el.setAttribute(prop, p[prop]);
  if (c) {
    if (typeof c === 'string') el.textContent = c;
    else if (c instanceof Array) {
      for (let elem of c) el.appendChild(elem);
    }
  }
  return el;
};
...
function init() {
  ...
  if (!window.contractAddress || !window.contractABI || !window.candidates)
    return table.appendChild(
      e('tr', [e('td', { colspan: 3 }, 'Ошибка. Сначала запустите контракт')])
    );
  ...
  if (window.testAccount) document.getElementById('from').innerText = window.testAccount;
...
function updateVotesForCandidate(candidate, element) {
  return contract.methods['totalVotesFor'](web3.utils.asciiToHex(candidate))
    .call()
    .then(votes => {
...
function voteForCandidate(candidate, from, callback) {
  try {
    return contract.methods['voteForCandidate'](web3.utils.asciiToHex(candidate))
      .send({
        from: from,
      })
      .then(callback);
